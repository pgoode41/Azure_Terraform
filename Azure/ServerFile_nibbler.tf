resource "azurerm_virtual_machine" "nibbler" {
    name                  = "nibbler"
    location              = "eastus"
    resource_group_name   = "${azurerm_resource_group.myterraformgroup.name}"
    network_interface_ids = ["${azurerm_network_interface.nibblernic.id}"]
...
resource "azurerm_network_interface" "nibblernic" {
    name                = "nibblernic"
    location            = "eastus"
    resource_group_name = "${azurerm_resource_group.myterraformgroup.name}"
    ...
    ip_configuration {
        ...
        public_ip_address_id          = "${azurerm_public_ip.nibblerPublicIP.id}"
    }
...
resource "azurerm_public_ip" "nibblerPublicIP" {
    name                         = "nibblerPublicIP"
    location                     = "eastus"
    resource_group_name          = "${azurerm_resource_group.myterraformgroup.name}"
